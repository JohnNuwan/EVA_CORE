...
```mermaid
graph TB
    subgraph INT["🔒 Docker Network: internal (No Internet Route)"]
        Agent["hermes-agent"]
        Dashboard["hermes-dashboard"]
        GtwInt["hermes-gtw (Gateway)"]
        Agent --> GtwInt
        Dashboard --> GtwInt
    end

    subgraph EXT["🌐 Docker Network: egress (Internet Capable)"]
        GtwExt["hermes-gtw (Dual-Homed)"]
        Proxy["egress-proxy<br/>(Squid / Envoy)"]
        Hosts["Allowlisted External Hosts"]
        GtwExt --> Proxy
        Proxy --> Hosts
    end
