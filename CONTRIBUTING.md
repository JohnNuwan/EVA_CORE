...
- **Search both open *and* merged PRs and issues** for your topic or error symptom — the duplicate-check in the PR template fires at review time, after you've already done the work:
  ```bash
  gh search issues --repo JohnNuwan/EVA_CORE "<your terms>"
  gh search prs --repo JohnNuwan/EVA_CORE --state all "<your terms>"
  ```
  Or use the web UI: [issues](https://github.com/JohnNuwan/EVA_CORE/issues?q=) · [PRs (all states)](https://github.com/JohnNuwan/EVA_CORE/pulls?q=is%3Apr).
