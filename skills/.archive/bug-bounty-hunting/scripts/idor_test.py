...
⚠ À n'utiliser QUE sur des cibles dans le scope d'un programme de bug bounty
  auquel tu participes, ou sur des sites de démo autorisés.
"""
import sys
import hashlib
...
def fetch(url):
    """Retourne (status, taille, hash_contenu, corps)."""
...
def main():
    ...
    if len(distinct) > 1:
        ...
        print("[i] Réponses identiques : pas d'IDOR évident (contenu uniforme/redirection).")
    else:
        codes = {}
        for (st, _), ids in seen.items():
            codes[st] = codes.get(st, 0) + len(ids)
